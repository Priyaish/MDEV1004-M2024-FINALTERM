...
const buildings_1 = __importDefault(require("../Models/buildings"));
function DisplayBuildingList(req, res, next) {
    buildings_1.default.find({})
        .then((data) => {
...
function DisplayBuildingById(req, res, next) {
    ...
    if (id.length !== 24) {
        ...
        buildings_1.default.findById({ _id: id })
            .then((data) => {
            if (data) {
...
function AddBuilding(req, res, next) {
    let building = new buildings_1.default({
        name: req.body.name,
        type: req.body.type,
    ...
    buildings_1.default.create(building)
        .then(() => {
...
function UpdateBuilding(req, res, next) {
    ...
    if (id.length !== 24) {
        ...
        let buildingToUpdate = new buildings_1.default({
            _id: id,
            name: req.body.name,
            type: req.body.type,
            dateBuilt: req.body.dateBuilt,
        ...
        buildings_1.default.updateOne({ _id: id }, buildingToUpdate)
            .then(() => {
...
function DeleteBuilding(req, res, next) {
    ...
    if (id.length !== 24) {
        ...
        buildings_1.default.deleteOne({ _id: id })
            .then(() => {
